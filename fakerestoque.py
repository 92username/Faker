...
produtos = ['Notebook', 'Smartphone', 'Tablet', 'Monitor', 'Teclado', 'Mouse', 'Impressora', 'Scanner', 'Câmera', 'Fone de Ouvido']
...
with open(output_file, mode='w', newline='', encoding='utf-8') as arquivo_csv:
    ...
    for i in range(1, num_itens + 1):  # Gerar a quantidade de itens especificada pelo usuário
        ...
        valor_total_estoque += valor_agregado  # Acumular o valor agregado no valor total do estoque
        escritor.writerow({
            'ID do Produto': i,
            ...
            'Data de Aquisição': fake.date_between(start_date='-2y', end_date='today'),
            'Fornecedor': fake.company(),
            'Localização no Armazém': f"Seção {random.randint(1,10)} - Prateleira {random.randint(1,5)}",
            'Número de Série': fake.unique.ean13() if categoria == 'Equipamentos Eletrônicos' else '',
            'Status': random.choice(status_opcoes),
            'Data de Validade': fake.date_between(start_date='today', end_date='+3y') if 'Bateria' in produto else '',
            'Observações': ''
        })
